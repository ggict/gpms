 ...
 */
package egovframework.example.cmmn.impl;
...
public class BaseDAO3 extends EgovAbstractDAO {
	private static final Logger LOGGER = LoggerFactory.getLogger(BaseDAO3.class);
     ...
     */
	@Resource(name = "sqlMapClientSQLServer")
	 ...
	 * 프로시저 콜을 실행한다.
	 * @param sqlMapId
	 * @param parameter recommend hashmap
	 * @return
	 */
 	public Object call(String sqlMapId, Object parameter){
 		try {
 			long startTime = System.currentTimeMillis();

 			getSqlMapClientTemplate().queryForObject(sqlMapId, parameter);

 			long endTime = System.currentTimeMillis();
 			if (LOGGER.isDebugEnabled()) {
 				LOGGER.debug("["+sqlMapId+"]query execute TIME : " + (endTime - startTime) + "(ms)]]");
 			}
 		}
 		catch(Exception e) {
			if (LOGGER.isDebugEnabled()) {
    ...
    public List procForList(String sql, Object objParam) {
 		ObjectMapper oMapper = new ObjectMapper();
		Map<String, Object> props = null;
		Object list = null;
		try {
			props = oMapper.convertValue(objParam, Map.class);
		} catch (Exception e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
			throw new RuntimeException (e);
		}
		return getSqlMapClientTemplate().queryForList(sql, props);
	}
