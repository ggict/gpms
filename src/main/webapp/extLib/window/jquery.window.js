...
$.Window = (function()  {
	...
	var ICON_WH = 25;               // window icon button width/height, in pixels. check "window_icon_button" style in css
...
//		return navigator.userAgent.match(/msie/i) && !window.XMLHttpRequest
	...
	function constructor(caller, options) {
		...
		var options = $.extend({
			...
			z: 1000,                      // [number:2000] the css z-index value
...
//			bookmarkable: true,           // [boolean:true] to control window with remote url could be bookmarked
			...
			,createNo : 0 				  // skc@muhanit.kr 추가. 해당 초기 minimize 위치값 지정
		}, options);
		...
		function initialize(instance) {
			...
			container.css("z-index", global_zindex++);
			...
			// add close button
			if( options.closable ) {
				headerFuncPanel.append( "<div title='' class='closeImg window_icon_button no-draggable'></div>" );
				headerFuncPanel.children(".closeImg").click(function() {

				    // JOY. 2017. 11. 23. 닫을 때 물어보기 action
				    if ( $(this).parent().parent().parent().find("iframe").contents().find("body").hasClass("cu") ) {
			...
			// add maximize button
			if( options.maximizable ) {
				headerFuncPanel.append( "<div title='' class='maximizeImg window_icon_button no-draggable'></div>" );
			...
			if( options.minimizable ) {
				headerFuncPanel.append( "<div title='' class='minimizeImg window_icon_button no-draggable'></div>" );
			...
			/**
			  window minimize 위치 고정
			  skc@muhanit.kr
			*/
			options.createNo = windowStorage.length>0?getMaxCreateNo() + 1:0;
		...
		/**
		 * minimize index 번호 세팅
		 * skc@muhanit.kr
		 */
		function setCreateNo(num) {
			options.createNo = num;
		}
		...
		/**
		 * minimize index 번호 조회
		 * skc@muhanit.kr
		 */
		function getCreateNo() {
			return options.createNo;
		}
		...
		function showOverlay() {
			...
			overlay.css('z-index', global_zindex+500);	// ***
		...
		function _adjustMinimizedPos(bImmediate, callback) {
			...
			targetCssStyle.top = targetCssStyle.top - defHeight;
			if( bImmediate ) {
				container.css(targetCssStyle);
				animating = false;
				if( $.isFunction(callback) ) {
		...
		*/
		...
		function select() {
			...
			if( maximized == false ) {
				...
				if( options.showModal )
					container.css('z-index', global_zindex+1000);
				else
					container.css('z-index', global_zindex++);
		...
		function minimize() {
			...
			miniStackIndex = options.createNo;
		...
		function restore() {
			...
			container.animate(targetCssStyle, setting.animationSpeed, 'swing', function() {
				...
				container.css('z-index', global_zindex++); // 최소화 되었다가 원복될때 포커스 되도록 처리
				showContent();
				header.unbind('click');
				adjustHeaderTextPanelWidth();
				adjustFrameWH();
		...
		/**
		 * 가장 큰 minimize index 번호 생성
		 * skc@muhanit.kr
		 */
		function getMaxCreateNo(){
			var maxNum = 0;
			for( var i=0, len=windowStorage.length; i<len; i++ ) {
				var wnd = windowStorage[i];
				if( maxNum < wnd.getCreateNo() ) {
					maxNum = wnd.getCreateNo();
				}
			}
	...
	return { // static public methods
		...
		minimizeAll: function() { // minimize all created windows
			for( var i=0, len=windowStorage.length; i<len; i++ ) {
				windowStorage[i].minimize();
			}
		},
		...
		setDefHeight : function(val){
			defHeight = val;
			for( var i=0, len=windowStorage.length; i<len; i++ ) {
				if(windowStorage[i].isMinimized()){
					var wh = getBrowserScreenWH();
					var wndId = windowStorage[i].getContainer()[0].id;
					$("#"+ wndId).css("top", wh.height - defHeight - 24);
