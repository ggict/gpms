...
/**
 * Constant: OpenLayers.Tile.Image.IFrame Mixin for tiles that use form-encoded
 * POST requests to get images from remote services. Images will be loaded using
 * HTTP-POST into an IFrame.
 * 
 * This mixin will be applied to <OpenLayers.Tile.Image> instances configured
 * with <OpenLayers.Tile.Image.maxGetUrlLength> set.
 */
OpenLayers.Tile.Image.IFrame = {

	/**
	 * Property: useIFrame {Boolean} true if we are currently using an IFrame to
	 * render POST responses, false if we are using an img element to render GET
	 * responses.
	 */
	useIFrame : null,

	/**
	 * Property: blankImageUrl {String} Using a data scheme url is not supported
	 * by all browsers, but we don't care because we either set it as css
	 * backgroundImage, or the image's display style is set to "none" when we
	 * use it.
	 */
	blankImageUrl : "data:image/gif;base64,R0lGODlhAQABAIAAAAAAAP///yH5BAEAAAAALAAAAAABAAEAQAIBRAA7",

	/**
	 * Method: draw Set useIFrame in the instance, and operate the image/iframe
	 * switch. Then call Tile.Image.draw.
	 * 
	 * Returns: {Boolean}
	 */
	draw : function() {
	...
	/**
	 * Method: getImage Creates the content for the frame on the tile.
	 */
	getImage : function() {
		if (this.useIFrame === true) {
			if (!this.frame.childNodes.length) {
				var eventPane = document.createElement("div"), style = eventPane.style;
			...
			if (parseFloat(navigator.appVersion.split("MSIE")[1]) < 9) {
				...
				// properly via DOM manipulation, so we need to do it on our own
				// with
				// this hack.
			...
			// some special properties to avoid scaling the images and
			// scrollbars
			// in the iframe
			iframe.scrolling = 'no';
	...
	/**
	 * Method: createRequestForm Create the html <form> element with width,
	 * height, bbox and all parameters specified in the layer params.
	 * 
	 * Returns: {DOMElement} The form element which sends the HTTP-POST request
	 * to the WMS.
	 */
	createRequestForm : function() {
		...
		// form element
		var imageSize = this.layer.getImageSize(), params = OpenLayers.Util
				.getParameters(this.url), field;
	...
	/**
	 * Method: setImgSrc Sets the source for the tile image
	 * 
	...
	/**
	 * Method: onImageLoad Handler for the image onload event
	 */
	...
	/**
	 * Method: createBackBuffer Override createBackBuffer to do nothing when we
	 * use an iframe. Moving an iframe from one element to another makes it
	 * necessary to reload the iframe because its content is lost. So we just
	 * give up.
	 * 
	 * Returns: {DOMElement}
	 */
	createBackBuffer : function() {
		...
		if (this.useIFrame === false) {
			backBuffer = OpenLayers.Tile.Image.prototype.createBackBuffer
					.call(this);
		}
		return backBuffer;
	}
