...
$.fn.jqGrid = function( pin ) {
	...
	return this.each( function() {
		...
		var p = $.extend(true,{
			...
			ExpandColumn: null,
			tree_root_level : 0,
			//prmNames: {page:"pageIndex",rows:"pageUnit", sort: "sidx",order: "sord", search:"_search", nd:"nd", id:"id",oper:"oper",editoper:"edit",addoper:"add",deloper:"del", subgridid:"id", npage: null, totalrows:"totalrows"},
		...
		addMulti = function(rowid,pos,irow,checked){
			var	v = "<input role=\"checkbox\" type=\"checkbox\""+" id=\"jqg_"+ts.p.id+"_"+rowid+"\" class=\"cbox\" name=\"jqg_"+ts.p.id+"_"+rowid+"\"" + (checked ? "checked=\"checked\"" : "")+"/><label for=\"jqg_"+ts.p.id+"_"+rowid+"\" class=\"hiddenLabel\">선택</label>",
		...
		setPager = function (pgid, tp){
			...
			if(ts.p.rowList.length >0){
				str = "<td dir='"+dir+"'>";
				str +="<select id='pageUnit' name='pageUnit' class='ui-pg-selbox' role='listbox'>";
			...
			if(dir==="rtl") { pgl += str; }
			if(ts.p.pginput===true) { pginp= "<td dir='"+dir+"'>"+$.jgrid.format(ts.p.pgtext || "","<input id='pageIndex' name='pageIndex' class='ui-pg-input' type='text' size='2' maxlength='7' value='0' role='textbox'/>","<span id='sp_1_"+$.jgrid.jqID(pgid)+"'></span>")+"</td>";}
			if(ts.p.pgbuttons===true) {
			...
			// *** 지노시스템 변경분 : 하단 페이저의 위치 구성
			if(ts.p.viewrecords===true) {$("td#"+pgid+"_left","#"+pgcnt).append("<div dir='"+dir+"' style='text-align:left' class='ui-paging-info'></div>");}
			...
			$('.ui-pg-selbox',"#"+pgcnt).bind('change',function() {
				// *** 지노시스템 변경분 : 500 라인 이상 검색시 comfirm 메시지 표시
				if( $(this).val()>=500 ) {
		...
		if(this.p.multiselect) {
			this.p.colNames.unshift("<input role='checkbox' id='cb_"+this.p.id+"' class='cbox' type='checkbox'/><label for='cb_"+this.p.id+"' class='hiddenLabel'>선택</label>");
		...
		if(ts.p.footerrow) {
			...
			if(ts.p.rownumbers) { grid.footers[0].className = 'jqgrid-rownum'; }
	...
	createEl : function(eltype,options,vl,autowidth, ajaxso) {
		...
		switch (eltype)
		{
			...
			case "button" :
				...
				if(eltype !== "button"){
					if(autowidth) {
						if(!options.size) { $(elem).css({width:"98%",height:"18px"}); }
