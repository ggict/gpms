...
GTMapLayerTool = OpenLayers.Class({
	...
	setLayerGroups : function(){
		var control = this;
		var layers = this.getThemeShowList('asc' , layers); // YYK 2018.02.09
		var groupsIdx = [];
						...
						if(this.layers[i][options.con]) {
						    ...
						    if (options.layer == undefined ) {
						        if ( table == "CELL_10" || table == "CELL_SECT" 
						        	|| table == "MV_ROAD_CELT0002" || table == "MV_ROAD_CELT0005" 
						        	|| table == "MV_ROAD_CELT0012" || table == "TN_POTHOLE"
						        	|| table == "DORO_TOT_GRS80_50_LINE" || table == "STA_TOT_GRS80_50"
						        	|| table == "M_CALS_T" || table == "CELL_10_4K") {
						        	
						            arr.push(this.layers[i]);
						        }
						    }
						    // 2018.02.09 YYK
						    else if (options.layer == 'cmptncLayer' ) {
						        if ( table == "CMPTNC_ZONE" || table == "CMPTNC_ZONE_2M"
						            || table == "CMPTNC_ZONE_05"
						            // 2019 고도화 사업 - 시군관리 편집 레이어 추가
						            || table == "PTH_CTSMNT"
						            || table == "PTH_STTMNT"
						            //|| table == "DORO_A001"
	 ...
	 **********************************************************************************/
	getThemeShowList : function(order, oLayer) {
		var options = {
			con : 'show',
			order : order,
			retAttr : 'theme'
	...
	getCloneLayers : function(obj) {
		...
		if(obj){
			for(key in obj) {
				arr.push(this.cloneObj(obj[key]));
			}
		}
		else{
			for(key in this.layers) {
				arr.push(this.cloneObj(this.layers[key]));
			}
		}
