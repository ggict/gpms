...
function fn_save() {
	...
	if( confirm('<spring:message code="warn.update.msg" />') ) {
		...
		COMMON_UTIL.cmFileFormSubmit("frm", "proc_frm", contextPath + "api/flawcntrwk/updateFlawCntrwk.do", "fn_save_callback");
...
function fn_save_callback(){
	...
	COMMON_UTIL.cmMovePage( "frm", "<c:url value='/flawcntrwk/selectFlawCntrwk.do'/>");
	...
	$.ajax({
		...
		,success: function(res){
			...
			if( res.length>0 ) {
				...
				// 포장재료 자동세팅
				if( res[0].RPAIR_MTHD_CODE!=null && $("#PAV_MATRL_ASCON_CODE").val() == "")
					$("#RPAIR_MTHD_CODE").val( res[0].RPAIR_MTHD_CODE );
				// 포장공법 자동세팅
				if( res[0].PAV_MATRL_ASCON_CODE && $("#PAV_MATRL_ASCON_CODE").val() == "")
...
function fn_change_track(selected_nscode,selected_banghyang,selected_charo) {
	$("#TRACK option").remove();
	...
	$.ajax({
		...
		,success: function(res){
			if( res.length>0 ) {
				...
				if( selected_charo!=null && selected_charo!='' )
					$("#TRACK").val( selected_charo );
			}
			else {
...
function fnSearchCompany(searchType){
	cmWindowOpen('공사업체 검색', "<c:url value='/company/searchCompany.do'/>?searchType="+searchType, 850, 600, false, $("#wnd_id").val(), 'center'); //밑에가있음..;
}
...
<input type="hidden" id="FLAW_CNTRWK_ID" name="FLAW_CNTRWK_ID" value="${flawCntrwkVO.FLAW_CNTRWK_ID}"/>
...
<div class="tabcont">
	...
	<div class="content">
		<div class="ml10 mr10">
			<table class="tbview" summary="포장 하자보수공사 정보를 수정한다.">
	            <caption>포장 하자보수공사 정보 수정</caption>
	            <colgroup>
	                <col width="15%" />
	                <col width="35%" />
	                <col width="15%" />
	            ...
	            <tbody>
					...
					<tr>
						...
						<td>
							<input type="hidden" name="FLAW_CO_NM" id="FLAW_CO_NM" value="${flawCntrwkVO.FLAW_CO_NM }" />
							<select name="FLAW_CO_NO" id="FLAW_CO_NO" onchange="fnChangeCompany('FLAW_CO')" alt="하자업체" class="select notnull">
								<option value="">선택</option>
								<c:forEach var="company" items="${companyList}">
								<option value="${company.CO_NO}" <c:if test = "${company.CO_NO == flawCntrwkVO.FLAW_CO_NO}"> selected="selected" </c:if>>${company.CO_NM}</option>
								</c:forEach>
							</select>
						</td>
						<th scope="row">하자검사일자<span class="fcred"> *</span></th>
						<td>
							<input type="text" name="FLAW_CHCK_DE" id="FLAW_CHCK_DE" value="${flawCntrwkVO.FLAW_CHCK_DE }" alt="하자검사일자" class="DT_DATE input notnull" />
						</td>
					</tr>
					<tr>
						<th scope="row">하자원인<span class="fcred"> *</span></th>
						<td colspan="3">
							<input type="text" name="FLAW_CAUSE" id="FLAW_CAUSE" value="${flawCntrwkVO.FLAW_CAUSE }" alt="하자원인" class="MX_100 CS_50 input notnull" placeholder="예) 포장두께 부족으로 인한 균열, 배수불량"/>
						</td>
					</tr>
					<tr>
						<th scope="row">보수위치<span class="fcred"> *</span></th>
						<td>
							<input type="text" name="RPAIR_LC" id="RPAIR_LC" value="${flawCntrwkVO.RPAIR_LC }" alt="보수위치" class="MX_100 CS_20 input notnull" />
						</td>
						<th scope="row">보수일자<span class="fcred"> *</span></th>
						<td>
							<input type="text" name="RPAIR_DE" id="RPAIR_DE" value="${flawCntrwkVO.RPAIR_DE }" alt="보수일자" class="DT_DATE input notnull" />
						</td>
					</tr>
					<tr>
						<th scope="row">보수연장(m)<span class="fcred"> *</span></th>
						<td>
							<input type="text" name="RPAIR_LEN" id="RPAIR_LEN" value="${flawCntrwkVO.RPAIR_LEN }" alt="보수연장" class="MX_8 CS_8 DT_INT input notnull" />
						</td>
						<th scope="row">보수폭(m)<span class="fcred"> *</span></th>
						<td>
							<input type="text" name="RPAIR_BT" id="RPAIR_BT" value="${flawCntrwkVO.RPAIR_BT }" alt="보수폭" class="MX_5 DD_2 CS_5 DT_FLOAT input notnull" />
						</td>
					</tr>
					<tr>
						<th scope="row">보수면적(㎡)<span class="fcred"> *</span></th>
						<td>
							<input type="text" name="RPAIR_AR" id="RPAIR_AR" value="${flawCntrwkVO.RPAIR_AR }" alt="보수면적" class="MX_10 DD_2 CS_10 DT_FLOAT input notnull" />
						</td>
					...
					<tr>
						...
						<td>
							표층　 : <input type="text" name="RPAIR_THICK_ASCON" id="RPAIR_THICK_ASCON" style="width: 120px;" value="${flawCntrwkVO.RPAIR_THICK_ASCON }" alt="포장두께"  class="MX_5 DD_2 CS_5 DT_FLOAT input notnull" placeholder="예) 5.5"/>
						</td>
						...
						<td>표층<span class="fcred"> *</span> : 
							...
							<input type="text" name="PAV_MATRL_ASCON_ETC" id="PAV_MATRL_ASCON_ETC" value="${flawCntrwkVO.PAV_MATRL_ASCON_ETC}" class="MX_100 CS_20 input" style="display: none;" />
						</td>
					</tr>
					<tr>
						<td>
							중간층 : <input type="text" name="RPAIR_THICK_CNTR" id="RPAIR_THICK_CNTR" style="width: 120px;" value="${flawCntrwkVO.RPAIR_THICK_CNTR}" alt="포장두께" class="MX_5 DD_2 CS_5 DT_FLOAT input notnull" placeholder="예) 5.5"/>
						</td>
						<td> 중간층 : 
							...
							<input type="text" name="PAV_MATRL_CNTR_ETC" id="PAV_MATRL_CNTR_ETC" value="${flawCntrwkVO.PAV_MATRL_CNTR_ETC}" class="MX_100 CS_20 input" style="display: none;"/>
						</td>
					</tr>
					<tr>
						<td>
							기층　 : <input type="text" name="RPAIR_THICK_BASE" id="RPAIR_THICK_BASE" style="width: 120px;" value="${flawCntrwkVO.RPAIR_THICK_BASE}" alt="포장두께" class="MX_5 DD_2 CS_5 DT_FLOAT input notnull" placeholder="예) 5.5"/>
						</td>
						<td> 기층　 : 
							...
							<input type="text" name="PAV_MATRL_BASE_ETC" id="PAV_MATRL_BASE_ETC" value="${flawCntrwkVO.PAV_MATRL_BASE_ETC}" class="MX_100 CS_20 input"  style="display: none;"/>
						</td>
					</tr>
					<tr> 
			...
			<div class="mt10 tc">
				<div class="fr">
					<c:if test="${sessionScope.userinfo.IS_ADMIN=='Y' || sessionScope.userinfo.DEPT_CODE==result.DEPT_CODE }">
						<a href="#" onclick="fn_save();" class="schbtn" >저장</a>
					</c:if>
					<a href="#" onclick="COMMON_UTIL.cmMoveUrl('<c:url value='flawcntrwk/selectFlawCntrwk.do'/>?CNTRWK_ID=${flawCntrwkVO.CNTRWK_ID}&FLAW_CNTRWK_ID=${flawCntrwkVO.FLAW_CNTRWK_ID}');" class="graybtn" >취소</a>
				</div>
			</div>
		</div>
