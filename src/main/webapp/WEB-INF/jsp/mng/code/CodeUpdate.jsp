	...
	<div class="ConBx">
		<div class="Pop_Section">
						...
						<table class="tbview" summary="상세코드 정보를 등록/수정합니다.">
							...
							<tbody>
								...
								<tr>
									...
									<td>${codeVO.UPDUSR_ID}</td>
								</tr>
								<tr>
									<th scope="row">수정 일시</th>
									<td><fmt:formatDate value="${codeVO.UPDT_DT}"
			...
			<div class="btnbx" style="margin-right: 15px;">
				<a href="#" class="schbtn" onclick="check = false; fn_code_save(); return false;">수정</a>
				<a href="#" class="schbtn"
					onclick="check = false; fn_code_delete(); return false;">삭제</a> <a href="#"
					class="schbtn"
...
function fn_code_save() {	
	...
	if( confirm('<spring:message code="warn.update.msg" />') ) {
		...
		var action = '<c:url value="/api/code/updateCode.do" />';
		$.ajax({
	        url: action,
...
function fn_code_delete(){
	var vform = $('#code_Regist-form');
	if(confirm("선택한 코드("+ COMMON_LANG.trimdata( vform.find('#CODE_NM').val()) +" )를 삭제하시겠습니까?")){
		var action = '<c:url value="/api/code/deleteCode.do" />';
