            ...
            <ul class="statsbx">
                <li style="float:none; width:97%">
                    <div class="graylinebx p10" style="width:100%;">
                        <div id="cntrwkDeptChart" class="cont_ConBx2" style="height: 320px; margin-left:20px;"></div>
                    </div>
                    <h4 style="text-align: center;background:none; width:100%;">관리기관별 포장공사 이력 통계</h4>
                </li>
            </ul>
        </div>
    </div>
...
function fnDeptSearch(deptCd,strDt,endDt,rw){
    ...
    fnCntrwkDeptSearch(deptCd,strDt,endDt,rw);//노선조회
}
...
//검색 처리
function fnCntrwkDeptSearch(deptCd,strDt,endDt,rw) {
    var data = {"SCH_DEPT_CODE" : deptCd, "SCH_STRWRK_DE" : strDt, "SCH_COMPET_DE" : endDt};
    
    $.ajax({
         url: '<c:url value="/"/>'+'api/cntrwkdtl/selectCntrwkDeptLenNewStats.do'
        ,type: 'post'
...
function drawDeptChart(dataList,rw){
    ...
    require([   'echarts','echarts/chart/bar', 'echarts/chart/line'   ],
            function (ec) {
                 var myChart = ec.init(document.getElementById('cntrwkDeptChart'));
                 myChart.setOption({
                        title   : { text: '관리기관별 통계'   },
...
function fnExcel() {
    if( confirm("엑셀 파일로 저장하시겠습니까?") ) {
        COMMON_UTIL.cmFormSubmit("frm", "proc_frm", "<c:url value='/cntrwkdtl/cntrwkDeptLenNewStatsExcel.do'/>", "");
    }
}
