...
<div id="sch_cnt01" class="tabcont">
    <h3 class="tc">(<fmt:formatDate pattern = "yyyy-MM-dd" value = "${rpairTrgetSlctnVO.SLCTN_DT}" />) 진행률(<c:out value="${rpairTrgetSlctnVO.PROGRS_RT}" />) 보수대상 선정 과정을 진행합니다.</h3>
    <div>
        <div class="m20">
...
$( document ).ready(function() {
    ...
    colModels[colIndex++]={name: "TRNSPORT_QY",  index: "TRNSPORT_QY", comments: "교통_량",  width: 80,align: "center", hidden: false, sortable :true, formatter: 'number', formatoptions: {decimalPlaces: 0, defaultValue: '0'}}; /*보수_대상_항목_그룹.교통_량 */
    colModels[colIndex++]={name: "POTHOLE_QY",  index: "POTHOLE_QY", comments: "포트홀_량",  width: 80,align: "center", hidden: false, sortable :true, formatter: 'number', formatoptions: {decimalPlaces: 0, defaultValue: '0'}}; /*보수_대상_항목_그룹.포트홀_량 */
...
// 		// 	var action = '<c:url value="/api/rpairtrgetgroup/selectRpairTrgetGroupCELLListRest.do"/>';
        ...
        // 		}
        // 	});
        var tables = [ "TN_RPAIR_TRGET_GROUP" ];
        var fields = [];
        var values = [];
        fields.push("TRGET_SLCTN_NO");
        values.push($("#TRGET_SLCTN_NO").val());

        if ( groupItemNo ) {
            fields.push("GROUP_ITEM_NO");
            values.push(groupItemNo);
        }
        ...
        MAP.fn_get_selectFeatureByAttr(parent.gMap, tables, [fields], [values],
                null, operator, attribute, true, 1, 0);
    }
    /*
     * 선정 ID(복수) 위치조회
     */
    ...
    function fn_select_cellSectIDS(cell_ids) {
        ...
        //
        MAP.fn_get_selectFeatureByAttr(parent.gMap, tables, [ fields ],
                [ values ], null, operator, attribute, true, 1, 0);
    }

    /**
     * 선정구간 위치조회
     */
    function fn_select_cellSectRange(route_code, direct_code, track, strtpt,
            endpt) {
        var tables = [ "CELL_SECT" ];
        ...
        parent.gMap.setLayerIndex(multiLayer, 0);
        //
        MAP.fn_get_selectFeatureByAttrBoundary(parent.gMap, tables, fields,
                values, null, "AND", attribute, true, 1, 0, boundaryFields,
                boundaryValues);
    }
    ...
    function drawChartDepts(dataList) {
        ...
        for (var i = 0; i < dataList.length; i++) {
            if (dataList[i].DEPT_NM != '' && dataList[i].DEPT_NM != null) {
                chartData.push({
                    "value" : dataList[i].AMOUNT_CALC,
                    "name" : dataList[i].DEPT_NM
                });
            }
        }
        ...
        require([ 'echarts', 'echarts/chart/pie' ], function(ec) {
            ...
            if (myChartDept != null && myChartMethod != null
                    && myChartAdmin != null) {
                hideRTProgress();
            }
    ...
    function drawChartMethods(dataList) {
        ...
        require([ 'echarts', 'echarts/chart/pie' ], function(ec) {
                ...
                series : [ {
                    ...
                    itemStyle : {
                        normal : {
                            ...
                            borderWidth : 1,
                            color : function(params) {
                                var colorList = [ '#507E32', '#5F933B',
                                        '#6BA543', '#88B76E', '#ACCA9E',
                                        '#C9DBC1', '#9BCA63', '#FAD860',
                                        '#F3A43B', '#60C0DD', '#FE2752',
                                        '#7EEA59', '#F2B731', '#D8C35C',
                                        '#41C0AE', '#FE8422', '#6EEA54',
                                        '#F1B541', '#D6C32C', '#73C1AE',
            ...
            myChartMethod = ec.init(document
                    .getElementById('rtChartMethodCode'));
    ...
    function drawChartAdmins(dataList) {
        ...
        require(
                ...
                function(ec) {
                    ...
                    myChartAdmin = ec.init(document
                            .getElementById('rtChartAdminCode'));
        ...
        $.ajax({
            ...
            success : function(data) {
                ...
                for (var i = 0; i < data.length; i++) {
                    txtHtml += "<option value='" + data[i].ROAD_NO + "'>"
                            + data[i].ROAD_NO_VAL + "</option>";
                }
    ...
    function showGridDepts(dataList) {
        ...
        $("#rtGridDeptCodeArea").jqGrid({
            ...
            colModel : [ {
                ...
                name : 'TOTAL_AMOUNT_CALC',
                index : 'TOTAL_AMOUNT_CALC',
                width : 80,
                align : "right",
                hidden : true,
                sortable : false,
        ...
        if (dataList.length != null && dataList.length > 0) {
            ...
            COMMON_UTIL.fn_set_grid_noRowMsg('rtGridDeptCodeArea', $(
                    "#rtGridDeptCodeArea").jqGrid("getGridParam").emptyrecords,
                    dataList.length);
        } else {
            COMMON_UTIL.fn_set_grid_noRowMsg('rtGridDeptCodeArea', $(
                    "#rtGridDeptCodeArea").jqGrid("getGridParam").emptyrecords,
    ...
    function showGridMethods(dataList) {
        ...
        if (dataList.length != null && dataList.length > 0) {
            total_amount_calc = dataList[0].TOTAL_AMOUNT_CALC;
            COMMON_UTIL
                    .fn_set_grid_noRowMsg(
                            'rtGridMethodCodeArea',
                            $("#rtGridMethodCodeArea").jqGrid("getGridParam").emptyrecords,
                            dataList.length);
        } else {
            COMMON_UTIL
                    .fn_set_grid_noRowMsg(
    ...
    function showGridAdmins(dataList) {
        ...
        if (dataList.length != null && dataList.length > 0) {
            ...
            COMMON_UTIL
                    .fn_set_grid_noRowMsg(
                            'rtGridAdminCodeArea',
                            $("#rtGridAdminCodeArea").jqGrid("getGridParam").emptyrecords,
                            dataList.length);
        } else {
            COMMON_UTIL
                    .fn_set_grid_noRowMsg(
     ...
     * 복수 확인 다이얼로그 열기
     */
    function fnSwitchDialog(confirmTitle, msgContents, btnTitles,
            btnCancleTitle, callOKs, callBackCancel) {
        var calcWidth = 140;
        ...
        if (btnTitles != null && callOKs != null) {
            for (var idx = 0; idx < btnTitles.length; idx++) {
                buttonTags += '<a href="#" class="schbtn" onclick="'
                        + callOKs[idx] + '();" style="min-width:70px" >'
                        + btnTitles[idx] + '</a>&nbsp;&nbsp;';
            }
        }
        if (btnCancleTitle != null && btnCancleTitle != ""
                && callBackCancel != null) {
            buttonTags += '<a href="#" class="schbtn" onclick="'
                    + callBackCancel + '();" style="min-width:70px" >'
                    + btnCancleTitle + '</a>&nbsp;&nbsp;';
        } else {
     ...
     * 확인 다이얼로그 열기
     */
    function fnConfirmDialog(confirmTitle, msgContents, btnOkTitle,
            btnCancleTitle, callOK, callBackCancel) {
        ...
        if (btnOkTitle != null && btnOkTitle != "" && callOK != null) {
            buttonTags = '<a href="#" class="schbtn" onclick="' + callOK
                    + '();" style="min-width:70px" >' + btnOkTitle
                    + '</a>&nbsp;&nbsp;';
        }
        if (btnCancleTitle != null && btnCancleTitle != ""
                && callBackCancel != null) {
    ...
    function fn_showChartPredct(slctn_year) {
        ...
        $.ajax({
            ...
            success : function(datas) {
                ...
                if (!datas.lcData && !datas.acData && !datas.rdData
                        && !datas.iriData && !datas.gpciData) {
                    fnShowMsgDialog("예측모델 조회", '데이터가 충분하지 차트가 표시되지 않습니다.',
                            function() {
                            });
                    ntcNo += 1;
                } else {
    ...
    function drawChartPredct(dataList) {
        ...
        var dataX = (dataList.lcData ? dataList.lcData.x
                : (dataList.acData ? dataList.acData.x
                        : (dataList.rdData ? dataList.rdData.x
                                : (dataList.iriData ? dataList.iriData.x
                                        : dataList.gpciData.x))));
        var series = [];
    ...
    function showGridPredct(dataList) {
        ...
        $("#rtGridPredctArea").jqGrid({
            ...
            colModel : [ {
                name : 'predctModelKndSe',
                index : 'predctModelKndSe',
                width : 200,
                align : "center",
                hidden : false,
                sortable : false
        ...
        for (i = 0; dataList.lcData && i < dataList.lcData.x.length; i++) {
            $("#rtGridPredctArea").jqGrid('addRowData', i + 1, {
                predctModelKndSe : "LC",
                x : dataList.lcData.x[i],
                y : dataList.lcData.y[i]
            });
        }
        for (i = 0; dataList.acData && i < dataList.acData.x.length; i++) {
            $("#rtGridPredctArea").jqGrid('addRowData', i + 1, {
                predctModelKndSe : "AC",
                x : dataList.acData.x[i],
                y : dataList.acData.y[i]
            });
        }
        for (i = 0; dataList.rdData && i < dataList.rdData.x.length; i++) {
            $("#rtGridPredctArea").jqGrid('addRowData', i + 1, {
                predctModelKndSe : "RD",
                x : dataList.rdData.x[i],
                y : dataList.rdData.y[i]
            });
        }
        for (i = 0; dataList.iriData && i < dataList.iriData.x.length; i++) {
            $("#rtGridPredctArea").jqGrid('addRowData', i + 1, {
                predctModelKndSe : "IRI",
                x : dataList.iriData.x[i],
                y : dataList.iriData.y[i]
            });
        }
        for (i = 0; dataList.gpciData && i < dataList.gpciData.x.length; i++) {
            $("#rtGridPredctArea").jqGrid('addRowData', i + 1, {
                predctModelKndSe : "GPCI",
                x : dataList.gpciData.x[i],
                y : dataList.gpciData.y[i]
            });
        }
    }
</script>
