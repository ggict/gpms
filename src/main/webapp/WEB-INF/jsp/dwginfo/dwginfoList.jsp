...
$( document ).ready(function() {
	...
	// 검색 목록 그리드 구성
	$("#gridArea").jqGrid({
		url: '<c:url value="/"/>'+'api/dwginfo/selectDwgInfoList.do'
		,autoencode: true
		...
		,ignoreCase: true
		,colNames:["ROAD_NO","노선 번호","구간 번호","도면시점(km)","도면종점(km)","도면 파일명"]
	   	,colModel:[
			...
			,{name:'SECT_ST',index:'SECT_ST', align:'center', width:80, sortable:true}
			,{name:'SECT_ED',index:'SECT_ED', align:'center', width:80, sortable:true}
...
function fn_download() {
	...
	if(obj.jqGrid("getGridParam","records")==0 || idx.length<1){
		alert("다운로드할 단위도면을 선택해주세요");
		return false;
	}
	
	for(var i=0; i<idx.length; i++){
		var value = obj.jqGrid('getCell', idx[i], 'DWG_NAME' );
		seqListArr.push( value );
	}
	
	seqList = seqListArr.join(",");
	$("#DWG_NAME").val(seqList);
	
	$.ajax({
		url: contextPath + 'api/dwginfo/checkDwgInfo.do'
		,type: 'post'
		,dataType: 'json'
		,data: {"ROAD_NO": $("#ROAD_NO").val(),"DWG_NAME": seqList}
		,success: function(data){
			...
			if(data.fileList.length > 0 ){
				...
				if(confirm(msg)){    	
					$("#frm").attr("action",contextPath + "api/dwginfo/downloadDwgInfo.do");
					$("#frm").attr("target","proc_frm");
					$("#frm").submit();
				}	
			}else{
				var msg = (idx.length*2) + "개의 단위도면을 다운로드 하시겠습니까?";
...
<input type="hidden" id="ROAD_NO" name="ROAD_NO" value="${dwgInfoVO.ROAD_NO }"/>
<input type="hidden" id="DWG_NAME" name="DWG_NAME" value=""/>
<div class="tabcont">
   <div class="bctab">
       	<ul>
           <li class="on"><a href="#" onclick="COMMON_UTIL.cmMoveUrl('dwginfo/selectDwgInfoList.do?ROAD_NO=${dwgInfoVO.ROAD_NO}')">단위도면 정보</a></li>
           <li><a href="#" onclick="COMMON_UTIL.cmMoveUrl('strdwg/selectStrDwgList.do?ROAD_NO=${dwgInfoVO.ROAD_NO}')" >구조물 도면 정보</a></li>
       	</ul>
       	<h3 class="info">
