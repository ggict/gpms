CREATE OR REPLACE FUNCTION gpms.prc_repair_target_range_select2(p_user_no numeric, p_trget_slctn_no numeric, p_slctn_budget numeric, p_dept_code text, p_decsn_mthd_1_rate numeric, p_decsn_mthd_2_rate numeric, p_decsn_mthd_3_rate numeric, p_decsn_mthd_4_rate numeric, p_decsn_mthd_5_rate numeric, p_mode text, OUT o_proccode text, OUT o_procmsg text)
 RETURNS record
 LANGUAGE plpgsql
AS $function$
...
BEGIN
    ...
    INSERT INTO TN_RPAIR_TRGET (
        ...
        SELECT
            ...
            , A.CALC_GPCI::NUMERIC                   CALC_GPCI                        /* 산정_GPCI */
            , A.CALC_PC_GRAD                         CALC_PC_GRAD                     /* 산정_포장상태등급 */
            , NULL                          CNTRWK_YEAR                      /* 공사_년도 */
            , NULL                                   TRNSPORT_QY                      /* 교통량 */
            , A.LEN                                  LEN                              /* 연장 */
            , A.AR                                   AR                               /* 면적 */
            , NULL                           RPAIR_MTHD_CODE                  /* 보수_공법_코드 */
        ...
        FROM (
            SELECT
                ...
                , NULL             DMG_VAL                          /* (임계값 - 산정_GPCI) 파손도_값 */
                , A.*
            FROM (
                SELECT
--                  FN_GET_CNTRWK_YEAR(A.ROUTE_CODE, A.DIRECT_CODE, A.TRACK::NUMERIC, A.STRTPT, A.ENDPT) CNTRWK_YEAR  /* 공사년도(최근공사년도) */
                    A.ROAD_L                         LEN                              /* 연장 */
                    , A.ROAD_A                         AR                               /* 면적 */
                    , NULL RPAIR_MTHD  /* 보수_공법_코드(TC_RPAIR_MTHD.RPAIR_MTHD_CODE)[Do Nothing(RPIR0001), 노화방지공법(RPIR0002), 균열 Sealing(RPIR0003)...] */
--                  , FN_GET_GPCI_LIMITVAL(A.CELL_TYPE, A.VMTC_GRAD, A.CALC_GPCI::NUMERIC, A.LC_IDX::NUMERIC, A.RD_IDX::NUMERIC, A.AC_IDX::NUMERIC, 'GPCI') THRHLD  /* 임계값 */
                    , A.*
                FROM (
                    SELECT
                        ...
                        , COALESCE(B.PC_GRAD, 999)   PC_GRAD                          /* 포장상태등급 */
                        , A.ROUTE_CODE               ROUTE_CODE                       /* 노선_코드 */
                        , A.DIRECT_CODE              DIRECT_CODE                      /* 방향 */
                        , A.TRACK                    TRACK                            /* 차로 */
                        , A.STRTPT                   STRTPT                           /* 시점 */
                        , A.ENDPT                    ENDPT                            /* 종점 */
                        , A.DEPT_CODE                DEPT_CODE                        /* 부서_코드 */
                        , A.CELL_TYPE                CELL_TYPE                        /* 섹션_구분 */
