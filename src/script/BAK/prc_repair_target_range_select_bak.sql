CREATE OR REPLACE FUNCTION PRC_REPAIR_TARGET_RANGE_SELECT2(P_USER_NO NUMERIC, P_TRGET_SLCTN_NO NUMERIC, P_SLCTN_BUDGET NUMERIC, P_DEPT_CODE TEXT, P_DECSN_MTHD_1_RATE NUMERIC, P_DECSN_MTHD_2_RATE NUMERIC, P_DECSN_MTHD_3_RATE NUMERIC, P_DECSN_MTHD_4_RATE NUMERIC, P_DECSN_MTHD_5_RATE NUMERIC, P_MODE TEXT, OUT O_PROCCODE TEXT, OUT O_PROCMSG TEXT)
RETURNS RECORD
LANGUAGE plpgsql
AS $function$
...
DECLARE
    ...
    LIST CURSOR FOR
    SELECT *
    FROM TN_MUMM_SCTN_SRVY_DTA A                                      /*** 최소_구간_조사_자료 테이블 ***/
    WHERE
        1 = 1
        AND A.DELETE_AT              = 'N'                            /* 삭제_여부 */
        AND A.USE_AT                 = 'Y'                            /* 사용_여부 */
        AND A.LAST_DTA_AT            = 'Y'                            /* 최종_자료_여부 */
    ;
            ...
            FROM (
                ...
                FROM (
                    ...
                    INNER JOIN TN_MUMM_SCTN_SRVY_DTA B                                /*** 최소_구간_조사_자료 테이블 ***/
                        ...
                        AND B.DTA_NO                 = V_EROW.DTA_NO                  /* 자료_번호(속도문제로 키값을 하나 걸어서 진행) */
                    LEFT OUTER JOIN (
                        SELECT
                        ...
                        WHERE
                            ...
                            AND AA.DTA_NO            = V_EROW.DTA_NO                  /* 자료_번호(속도문제로 키값을 하나 걸어서 진행) */
                        GROUP BY
                            AA.ROUTE_CODE                                             /* 노선코드 */
                            , AA.DIRECT_CODE                                          /* 방향 */
                            , AA.TRACK                                                /* 차로 */
