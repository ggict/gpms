		...
		<div class="ConBx">
			<div class="Pop_Section">
				<div class="PopTitBx">
					...
					<div class="content" style="padding: 0px 10px;" id="company_Regist-div">
						<form:form commandName="companyVO" id="company_Regist-form">
							...
							<tbody>
								<tr>
									<th scope="row"><span class="fcred">* </span>업체 번호</th>
									<td><form:input path="CO_NO" maxLength="4"
											cssClass="input notnull" readonly="true" /></td>
								</tr>
								<tr>
								...
								<tr>
									...
									<td><form:input type="tel" path="BIZ_NO" maxLength="12"
											cssClass="input notnull" /> <form:errors path="BIZ_NO" /></td>
								</tr>
								...
								<tr>
									<th scope="row">업체 상태 코드</th>
									<td><form:select path="CO_STTUS_CODE" style="width: 99%;">
								...
								<tr>
									...
									<td>${companyVO.UPDUSR_ID}</td>
								</tr>
								<tr>
									<th scope="row">수정 일시</th>
									<td><fmt:formatDate value="${companyVO.UPDT_DT}"
				...
				<div class="btnbx" style="margin-right: 15px;">
					...
					<a href="#" class="schbtn" onclick="fn_company_save(); return false;">수정</a>
					<a href="#" class="schbtn" onclick="fn_company_delete(); return false;">삭제</a>
					<a href="#" class="schbtn" onclick="if(confirm('수정된 내용이 사라집니다. 정말 취소하시겠습니까?')) {javascript:fn_close_dialog();} else {return;}">닫기</a>
				</div>
			</div>
		</div>
	</div>
...
$(document).ready(function(){
	...
	$("input[type='tel']").change(function(event){
		//event.preventDefault();
		if(COMMON_UTIL.validatePhone(this.id)==false){
...
function fn_company_save() {	
	...
	*/
	...
	if( confirm('<spring:message code="warn.update.msg" />') ) {
		...
		var action = '<c:url value="/api/company/updateCompany.do" />';
		$.ajax({
	        url: action,
...
/* 글 등록 function */
function fn_company_delete() {	
	var vform = $('#company_Regist-form');
 	
	if(confirm('<spring:message code="warn.delete.msg" />')){

		var action = '<c:url value="/api/company/deleteCompany.do" />';
		$.ajax({
	        url: action,
	        contentType: 'application/json',
	        data: JSON.stringify( vform.cmSerializeObject())  ,
