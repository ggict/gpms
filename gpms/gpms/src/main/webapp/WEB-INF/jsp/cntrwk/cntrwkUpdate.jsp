...
<!-- 기타 구분 파라메터 -->
<%-- <input type="hidden" id="CELL_TYPE" name="CELL_TYPE" value="<c:out value="${cntrwkVO.CELL_TYPE}"/>"/> --%>
<input type="hidden" id="EXT_TAB" name="EXT_TAB" value="<c:out value="${param.EXT_TAB}"/>"/>
<input type="hidden" id="FILE_NO" name="FILE_NO" value="<c:out value="${cntrwkVO.FILE_NO}"/>"/>
<div>
	<!-- Content -->
	<div class="posiR">
       	<ul class="ctab_menu">
       	...
       	<h5 class="info" style="left: 310px;">
			<a href="#" class="whitebtn dpib ml10 vm" onclick="COMMON_UTIL.cmMoveUrl('cntrwk/selectCntrwkView.do?CNTRWK_ID=${cntrwkVO.CNTRWK_ID}')"><img src="<c:url value='/images/ic_back.png'/>" alt="뒤로가기" title="뒤로가기" /></a>
		</h5>
		<p class="location">
	        <span>포장공사 이력관리</span>
	        <span>포장공사 이력관리</span>
	        <strong>포장공사 기본정보 수정</strong>
	    </p>
    </div>
    <!-- Content -->
	<div class="ctab_wrap">
		<div class="tabcont">
	    	<div class="scroll" style="height:240px;">
		        <table class="tbview" summary="포장공사 기본정보를 수정합니다.">
		            <caption>포장공사 기본정보 수정</caption>
		            <colgroup>
		                <col width="15%" />
		                <col width="35%" />
		                <col width="15%" />
		            ...
		            <tbody>
		                <tr>	
							<th scope="row">사업소<span class="fcred"> *</span></th>
							<td>
							...
							<th scope="row">공사년도<span class="fcred"> *</span></th>
							<td>
								...
								<select name="HT_SE" id="HT_SE" style="width:100px; margin-left: 5px;" class="select">
								<option value="">== 전체 ==</option>
						...
						<tr>
							...
							<th scope="row">공사구분<span class="fcred"> *</span></th>
							<td>
							...
							<th scope="row">공사분류<span class="fcred"> *</span></th>
							<td>
						...
						<tr> 
							<th scope="row">공사명<span class="fcred"> *</span></th>
							<td colspan="3">
								<label for="FULL_CNTRWK_NM"></label>
								<input type="text" name="FULL_CNTRWK_NM" id="FULL_CNTRWK_NM" maxlength="200" value="<c:out value="${cntrwkVO.FULL_CNTRWK_NM}"/>" class="MX_200 CS_50 input" style="width: 99.4%;" />	<!-- placeholder="예) 2016년 노후포장도로 정비공사" -->
								<label for="DETAIL_CNTRWK_NM"></label>
								<input type="text" name="DETAIL_CNTRWK_NM" id="DETAIL_CNTRWK_NM" maxlength="200" value="<c:out value="${cntrwkVO.FULL_CNTRWK_NM}"/>" class="MX_200 CS_50 input" style="width: 99.4%; display: none;" />	<!-- placeholder="예) 2016년 노후포장도로 정비공사" -->
							</td>
						</tr>
						<tr>
							...
							<td>
								...
								<input type="text" name="STRWRK_DE" id="STRWRK_DE" value="<c:out value="${cntrwkVO.STRWRK_DE}"/>" class="DT_DATE input" style="margin-right: 5px;" />
								-
								<label for="COMPET_DE"></label>
								<input type="text" name="COMPET_DE" id="COMPET_DE" value="<c:out value="${cntrwkVO.COMPET_DE}"/>" class="DT_DATE input" style=" margin: 0px 5px;" />
							</td>
							<th scope="row">시공사<span class="fcred"> *</span></th>
							<td>
								<input type="hidden" name="CNSTRCT_CO_NM" id="CNSTRCT_CO_NM" value="${cntrwkVO.CNSTRCT_CO_NM }" />
								...
								<select name="CNSTRCT_CO_NO" id="CNSTRCT_CO_NO" onchange="fnChangeCompany('CNSTRCT_CO')" class="select" style="width: 100%;">
									...
									<option value="${company.CO_NO}" <c:if test = "${company.CO_NO == cntrwkVO.CNSTRCT_CO_NO}"> selected="selected" </c:if> >${company.CO_NM}</option>
									</c:forEach>
								</select>
							</td>
						</tr>
						<%-- <tr>
							...
							<td colspan="3">
								...
								<input type="text" name="FLAW_BEGIN_DE" id="FLAW_BEGIN_DE" value="<c:out value="${cntrwkVO.FLAW_BEGIN_DE}"/>" class="DT_DATE input" />
								-
								<label for="FLAW_END_DE"></label>
								<input type="text" name="FLAW_END_DE" id="FLAW_END_DE" value="<c:out value="${cntrwkVO.FLAW_END_DE}"/>" class="DT_DATE input" />
							</td>
						</tr> --%>
						<%-- <tr>							
							...
							<th scope="row">감리사</th>
							<td>
								<input type="hidden" name="SPRVISN_CO_NM" id="SPRVISN_CO_NM" value="${cntrwkVO.SPRVISN_CO_NM }"  />
								...
								<select name="SPRVISN_CO_NO" id="SPRVISN_CO_NO" onchange="fnChangeCompany('SPRVISN_CO')" class="select" style="width: 100%;">
									...
									<c:forEach var="company" items="${companyList}">
									<option value="${company.CO_NO}" <c:if test = "${company.CO_NO == cntrwkVO.SPRVISN_CO_NO}"> selected="selected" </c:if> >${company.CO_NM}</option>
									</c:forEach>
								</select>
							</td>
						</tr> --%>
						<tr>
							<th scope="row">시공사 대표자<span class="fcred"> *</span></th>
							<td>
								<label for="CNSTRCT_CO_RPRSNTV_NM"></label>
								<input type="text" name="CNSTRCT_CO_RPRSNTV_NM" id="CNSTRCT_CO_RPRSNTV_NM" value="<c:out value="${cntrwkVO.CNSTRCT_CO_RPRSNTV_NM}"/>" class="MX_20 CS_25 input" style="width: 98.5%;" />
							</td>
							<th scope="row">시공사 대표번호<span class="fcred"> *</span></th>
							<td>
								<label for="CNSTRCT_CO_TELNO"></label>
								<input type="text" name="CNSTRCT_CO_TELNO" id="CNSTRCT_CO_TELNO" value="<c:out value="${cntrwkVO.CNSTRCT_CO_TELNO}"/>" class="MX_13 CS_25 input" style="width: 98.5%;" />
							</td>
							<%-- <th scope="row">감리사 대표자</th>
							<td>
								<label for="SPRVISN_CO_RPRSNTV_NM"></label>
								<input type="text" name="SPRVISN_CO_RPRSNTV_NM" id="SPRVISN_CO_RPRSNTV_NM" value="<c:out value="${cntrwkVO.SPRVISN_CO_RPRSNTV_NM}"/>" class="MX_20 CS_25 input" style="width: 98.5%;" />
							</td> --%>
						</tr>
						<%-- <tr>
							...
							<td>
								...
								<input type="text" name="SPRVISN_CO_RPRSNT_TELNO" id="SPRVISN_CO_RPRSNT_TELNO" value="<c:out value="${cntrwkVO.SPRVISN_CO_RPRSNT_TELNO}"/>" class="MX_15 CS_25 input" style="width: 98.5%;" />
							</td>
						</tr> --%>
						<tr>
						...
						<tr>
							...
							<th scope="row">포장두께<span class="fcred"> *</span></th>
							<td>
								<input type="text" name="RPAIR_THICK_DC" id="RPAIR_THICK_DC" value="<c:out value="${cntrwkVO.RPAIR_THICK_DC}"/>" class="MX_50 CS_25 input" placeholder="예) 표층 5~10cm, 기층 6cm" style="width: 99.4%;"/><!-- cm -->
							</td>
						</tr>
						...
						<tr>
							...
							<td colspan="3">
								...
								<c:import url="/common/selectFileList.do" >
									<c:param name="FILE_NO" value="${cntrwkVO.FILE_NO}" />
	        ...
	        <div class="mt10 tc">
	            <div class="fr mr10">
	            	<c:if test = "${action_flag == 'UPDATE'}">
						<c:if test="${sessionScope.userinfo.IS_ADMIN=='Y' || ( sessionScope.userinfo.DEPT_CODE==cntrwkVO.DEPT_CODE)}">
							<a href="#" onclick="check = false; fnSave();" class="schbtn" >저장</a>
						</c:if>
						<c:if test="${sessionScope.userinfo.IS_EXT == 'Y'}">
							<a href="#" onclick="check = false; fnSave();" class="schbtn" >저장</a>
						</c:if>
						<a href="#" onclick="COMMON_UTIL.cmMovePage('frm','<c:url value='/cntrwk/ibmView.do'/>');" class="graybtn" >취소</a>
					</c:if>
					<c:if test = "${action_flag != 'UPDATE'}">
						<a href="#" onclick="check = false; fnSave();" class="schbtn" >저장</a>
						<c:if test="${sessionScope.userinfo.IS_EXT == 'Y'}">
					...
					<a href="#" onclick="COMMON_UTIL.cmMovePage('frm','<c:url value='/cntrwk/selectCntrwkView.do'/>');" class="graybtn" >취소</a>
					<a href="#" class="graybtn" onclick="check = false; fnDelete();">삭제</a>
	           	</div>
	        </div>
		</div>
	</div>
	...
	if(!validateCntrwk("frm")){
		...
		if( confirm(msg) ) {
			...
			try {
				...
				COMMON_UTIL.cmFileFormSubmit("frm", "proc_frm", "<c:url value='/cntrwk/updateCntrwk.do'/>","fnSaveCallback");
			} catch(E) {
 ...
 */
	...
	if(COMMON_LANG.trimdata( vform.find('#CNSTRCT_CO_NM').val()).length>200){
		alert("시공사 값은 최대 200자까지 입력할 수 있습니다.");
		return false;
	}	
 ...
 */
	...
	if(COMMON_LANG.trimdata( vform.find('#RPAIR_THICK_DC').val()).length>200){
		alert("포장 두께 설명 값은 최대 200자까지 입력할 수 있습니다.");
		return false;
	}	
