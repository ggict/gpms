...
GMap = OpenLayers.Class(OpenLayers.Map, {
	...
	/**********************************************************************************
	 * 함수명 : cleanMeasure
	 * 설 명 : 벡터레이어를 제외한 지도 위의 모든 도형(사용자 그래픽)과 팝업을 삭제.
	 * 인 자 :
	 * 사용법 : cleanMeasure
	 * 작성일 : 2018.02.28
	 * 작성자 : 유영기 YYK
	 * 수정일				수정자			수정내용
	 * ----------------------------------------------------------------------
	 * 2018.02.28		YYK		최초 생성
	 *
	 **********************************************************************************/
	cleanMeasure : function() {
		var currControls = [];
		for(var i in this.controls) {
			if(this.controls[i].active) {
				currControls.push(this.controls[i]);
		...
		for(var i in this.layers) {
			if(this.layers[i].name == "GPathMeasure" || this.layers[i].name == "GPolygonMeasure" ) {
				this.layers[i].removeFeatures(this.layers[i].features);
			}
		}
		this.removeAllPopups();
	...
	setCenterScale : function(lonlat, scale) {
		if(scale) {
			...
			//this.setCetner(lonlat); 2018.05.25 유영기. 오타수정
