...
/**
 * Class: GDynamicMeasure
 * Allows for drawing of features for measurements.
 *
 * Inherits from:
 *  - <OpenLayers.Control.Measure>
 */
GDynamicMeasure = OpenLayers.Class(OpenLayers.Control.Measure, {
     ...
     *     "labelArea". Default is <GDynamicMeasure.styles>.
     */
    styles: null,
    ...
    initialize: function(handler, options) {
        ...
        var defaultStyles = GDynamicMeasure.styles;
    ...
    activate: function() {
        ...
        if (response) {
            ...
            var _optionsStyles = this.styles || {},
                _defaultStyles = GDynamicMeasure.styles,
                _self = this;
            ...
            this.map.events.register('mouseout', this, this.mapMouseOut);
        }
        return response;
    },
    ...
    deactivate: function() {
        ...
        if (response) {
            ...
            this.map.events.unregister('mouseout', this, this.mapMouseOut);
        }
        return response;
    },

	
	fn_set_mouseDirection : function(x, y, px, py){
		...
		if(py - y > 0){
			this.mouseDirection.y = "top";			
		}
		else{
			this.mouseDirection.y = "bottom";			
		}
		
	},
	
	fn_init_mouseDirection : function(){
		this.mouseDirection = {
				"x" : "",
				"Y" : ""
		};
	...
	mapMouseOut: function(){
		//console.log(this.mouseDirection.x  +":" + this.mouseDirection.y + " / " + this.curMouseX + ":" + this.curMouseY + " / " + this.preMouseX + ":" + this.preMouseY  + " / " + $( window ).width() + ":" + $( window ).height());
		if(this.movingExtent && this.mapAutoMoveMode){
			map.zoomToExtent(this.movingExtent);
		}
	},
	...
	mapMouseMove: function(){
		...
		if(this.preMouseX && this.preMouseY && (parseInt(this.preMouseX) - parseInt(this.curMouseX) !== 0) && (parseInt(this.preMouseY) - parseInt(this.curMouseY) !== 0)){
			this.fn_set_mouseDirection(this.curMouseX, this.curMouseY, this.preMouseX, this.preMouseY);
		}

		var windowLeftPos = $("#left").width();
		var windowTopPos = $("#header").height();
		
	    var mapExtent = map.getExtent();
	    var moveDistX = parseInt(mapExtent.getWidth()/3,10);
	    var moveDistY = parseInt(mapExtent.getHeight()/3,10);
	    
	    if(this.mouseDirection.x === "right" && (this.curMouseX >= (windowWidth-35))) //마우스가 우측 끝부분에 거의 닿았을 때
	    {
	    	this.movingExtent = new OpenLayers.Bounds(mapExtent.left+moveDistX, mapExtent.bottom, mapExtent.right+moveDistX, mapExtent.top);
	    }
	    else if(this.mouseDirection.y === "bottom" && (this.curMouseY >= (windowHeight-35))) //마우스가 하단 끝부분에 거의 닿았을 때
	    {
	    	this.movingExtent = new OpenLayers.Bounds(mapExtent.left, mapExtent.bottom-moveDistY, mapExtent.right,  mapExtent.top-moveDistY);
	    }
	    else if(this.mouseDirection.x === "left" && (this.curMouseX <= (windowLeftPos+35))) //마우스가 좌측 끝부분에 거의 닿았을 때
	    {
	    	this.movingExtent = new OpenLayers.Bounds(mapExtent.left-moveDistX, mapExtent.bottom, mapExtent.right-moveDistX, mapExtent.top);
	    }
	    else if(this.mouseDirection.y === "top" && this.curMouseY <= windowTopPos+35) //마우스가 상단 끝부분에 거의 닿았을 때
    	{
	    	this.movingExtent = new OpenLayers.Bounds(mapExtent.left, mapExtent.bottom+moveDistY, mapExtent.right, mapExtent.top+moveDistY);
    	}
	    else{
	    	this.movingExtent = null;
    	}
    ...
    callbackPoint: function(point, geometry) {
        ...
        if(dynamicObj.countSegments > 0)
        	this.mapAutoMoveMode = true;
...
/**
 * Constant: GDynamicMeasure.positions
