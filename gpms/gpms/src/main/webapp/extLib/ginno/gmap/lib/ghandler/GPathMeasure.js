...
//					+ '</div>';
	        	this.popup = new GPopup("measurePopup", lonlat, null, contentHtml, new OpenLayers.Pixel(10,30));
        ...
        //처음 클릭이 아닐 경우
	    	...
	    	//contentHtml = "<div class='olControlMeasurePopup olControlMeasurePopupDefault'>"+ this.partDist[0] + this.partDist[1] + "</div>";
	    	popup = new GPopup("measurePopup", lonlat, null, contentHtml, new OpenLayers.Pixel(5,5));
	    	//클릭 횟수 증가
            this.count += 1;
	    }
...
//					$(this.popup.contentDiv).find(".MeasureAllDist .measureResUnit").text(" " + allDist[1]);
	...
	dblclick: function(evt) {
        ...
        var lonlat = this.map.getLonLatFromPixel(evt.xy);
        var allDist = this.measureDistanceAll();
		var contentHtml = "<div class='olControlMeasurePopup olControlMeasurePopupEnd'>총거리 : "+ allDist[0] + allDist[1] + "</div>";
	    var popup = new GPopup("measurePopup", lonlat, null, contentHtml, new OpenLayers.Pixel(-30, 30));
		this.map.addPopup(popup);
		popup.type = "measure" + GPathMeasureINDEX;
		popup.updateSize();
